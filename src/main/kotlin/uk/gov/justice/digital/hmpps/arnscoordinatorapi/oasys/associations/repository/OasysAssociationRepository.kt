...
interface OasysAssociationRepository : JpaRepository<OasysAssociation, Long> {
  ...
  @Query("SELECT * FROM coordinator.oasys_associations WHERE entity_type in ('AAP_PLAN', 'AAP_SAN') AND oasys_assessment_pk = :oasysAssessmentPk", nativeQuery = true)
  fun findAllByOasysAssessmentPkIncludingDeleted(oasysAssessmentPk: String): List<OasysAssociation>

  @Query("SELECT * FROM coordinator.oasys_associations WHERE entity_type in ('AAP_PLAN', 'AAP_SAN') AND oasys_assessment_pk = :oasysAssessmentPk AND deleted IS TRUE", nativeQuery = true)
  ...
  @Query("SELECT * FROM coordinator.oasys_associations WHERE entity_type in ('AAP_PLAN', 'AAP_SAN') AND entity_uuid = :entityUuid", nativeQuery = true)
